...
def parse_xml_test_warpper(xml_tests):
    """Tests the input list of xmls
    """
    for xml_file, xml_packet in xml_tests:
        # Parse the file
        if xml_file is None:
            xml_loc = None
            yaml_loc = os.path.join(os.path.dirname(
                __file__), 'test_events/Fermi_GRB.yaml')
        else:
        ...
        with open(yaml_loc, 'r') as stream:
            ...
            expected_trig['event_observed'] = str(
                expected_trig['event_observed'])
            expected_trig['xml'] = None
            # Put the packet through prettystr so they match
            expected_trig['packet'] = voeventparse.prettystr(
                voeventparse.loads(expected_trig['packet'].encode()))
...
def test_parse_grb_event():
    xml_tests = [
        ...
        ('Fermi_GRB.xml', None),
        ('Swift_BAT_GRB_Pos_fail.xml', None),
