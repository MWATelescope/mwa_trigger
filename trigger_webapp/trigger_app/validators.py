...
def atca_freq_bands(min_freq, max_freq, freq, field_name):
    if freq + 1000 > max_freq:
        raise forms.ValidationError({field_name: gettext(f"{field_name} error: A centre frequency of {freq} MHz would have a maximum above {max_freq} MHz which is outside the bands frequency range.")})
    if freq - 1000 < min_freq:
        raise forms.ValidationError({field_name: gettext(f"{field_name} error: A centre frequency of {freq} MHz would have a minimum below {min_freq} MHz which is outside the bands frequency range.")})
